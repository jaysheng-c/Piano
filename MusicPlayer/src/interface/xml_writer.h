...
public:
    ...
    xmlNodePtr CreateNode(const std::string& name);
    void AppandChild(xmlNodePtr parent, xmlNodePtr cur);
    int SetNodePop(xmlNodePtr node, std::list<Property> props);
    int SetNodePop(xmlNodePtr node, const std::string& name, const std::string& value);
    int SetNodeText(xmlNodePtr node, const std::string& text);
protected:
